...
def generate_image(prompt):
    ...
    try:
        ...
        if 'logo' in clean_prompt.lower():
            clean_prompt += ", professional logo design, vector style, clean, bold typography, centered text, high contrast, white background"
        elif 'icon' in clean_prompt.lower():
            clean_prompt += ", modern icon design, minimalist, flat design, white background"
        else:
            clean_prompt += ", high quality, detailed, professional, 4k"
        
        # Try to emphasize text rendering
        if any(word in clean_prompt.lower() for word in ['logo', 'name', 'text', 'brand']):
            clean_prompt += ", clear readable text, typography centered, bold letters, professional font"
        
        # Try Hugging Face Inference API (Stable Diffusion XL - better at text)
        try:
            ...
            if response.status_code == 200:
                return "Hugging Face SDXL", response.content
        except:
            pass
        
        # Fallback to Pollinations.ai with Flux model
        encoded_prompt = urllib.parse.quote(clean_prompt)
        image_url = f"https://image.pollinations.ai/prompt/{encoded_prompt}?width=1024&height=1024&nologo=true&seed={hash(prompt)}&model=flux"
        ...
        if response.status_code == 200:
            ...
            image_url_fallback = f"https://image.pollinations.ai/prompt/{encoded_prompt}?width=1024&height=1024&nologo=true&seed={hash(prompt)}"
            response_fallback = requests.get(image_url_fallback, timeout=20)
            if response_fallback.status_code == 200:
                return image_url_fallback, response_fallback.content
